...
async function main() {
    ...
    const postB = await prisma.post.create({
        data: {
            ...
            content: 'This is our first post in Org B! We focus on technology and business solutions. Excited to share our journey with you.',
            version: 1,
            revisions: {
                create: [{ version: 1, content: 'This is our first post in Org B! We focus on technology and business solutions. Excited to share our journey with you.' }]
            },
            postTags: {
                create: [{ tagId: tagTech.id }, { tagId: tagBusiness.id }]
            },
            comments: {
                create: [{ authorId: editorB.id, content: 'Excellent introduction! Looking forward to more tech insights.' }]
            }
        },
        include: { revisions: true, comments: true, postTags: { include: { tag: true } } }
    });
    ...
    const formB = await prisma.form.create({
        data: {
            ...
            schema: { type: 'object', properties: { issue: { type: 'string' }, priority: { type: 'string' } } },
            fields: {
                create: [
    ...
    await Promise.all([
        ...
        // Org B file assets
        prisma.fileAsset.create({ data: { url: 'https://example.com/post-b.png', mimeType: 'image/png', postId: postB.id } }),
        prisma.fileAsset.create({ data: { url: 'https://example.com/submission-b.txt', mimeType: 'text/plain', submissionId: submissionB.id } }),
    ]);
    ...
    await prisma.auditLog.createMany({
        data: [
            ...
            // Org B audit logs
            { organizationId: orgB.id, userId: adminB.id, action: 'POST_CREATED', resource: 'Post', resourceId: postB.id },
            { organizationId: orgB.id, userId: editorB.id, action: 'COMMENT_CREATED', resource: 'Comment', resourceId: postB.comments[0].id },
            { organizationId: orgB.id, userId: adminB.id, action: 'FORM_CREATED', resource: 'Form', resourceId: formB.id }
        ]
    });
