    ...
    return function MockAdminJobsPage() {
        ...
        const runTagStats = async () => {
            try {
                ...
                await axios.post(`${api('/admin/jobs/tag-stats/run')}`, {}, {
                    headers: {
                        'Accept': 'application/json',
                        'Authorization': `Bearer ${actualToken}`,
                        'x-org-id': actualOrgId,
        ...
        const runPreview = async (postId: string) => {
            try {
                ...
                const response = await axios.post(`${api(`/admin/jobs/post-preview/${postId}`)}`, {}, {
                    headers: {
                        'Accept': 'application/json',
                        'Authorization': `Bearer ${actualToken}`,
                        'x-org-id': actualOrgId,
...
jest.mock('@portfolio-grade/ui-kit', () => ({
    ...
    Table: ({ columns, data, loading, emptyMessage, theme }: {
        columns: any[];
        data: any[];
        loading: boolean;
        emptyMessage: string;
...
describe('AdminJobsPage', () => {
    ...
    it('should render the page with correct title and description', () => {
        ...
        expect(screen.getByText('Manage background jobs and view system statistics.')).toBeInTheDocument();
    });
    ...
    it('should render tag statistics section', () => {
        ...
        expect(screen.getByText('View tag usage statistics across your organization. Click "Refresh Tag Stats" to recalculate.')).toBeInTheDocument();
    });
    ...
    it('should render post preview jobs section', () => {
        ...
        expect(screen.getByText('Generate previews for posts. This is useful for creating thumbnails or summaries.')).toBeInTheDocument();
    });
