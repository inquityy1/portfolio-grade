...
jest.mock('./CreateUserPage', () => {
    ...
    const { Container, Button, Label, Input, Field, Select, Alert } = require('@portfolio-grade/ui-kit');
    ...
    return function MockCreateUserPage() {
            ...
            try {
                ...
                await axios.post(`${api('/users')}`, {
                    ...
                    headers: {
                        ...
                        'idempotency-key': `create-user-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
                    }
                });

                setSuccess(`User ${formData.name} (${formData.email}) created successfully with ${formData.role} role!`);
        ...
        return (
            <Container maxWidth="600px">
                ...
                {error && (
                    <Alert style={{ marginBottom: 16, color: 'tomato' }}>
                        {error}
                    </Alert>
                )}

                {success && (
                    <Alert style={{ marginBottom: 16, color: 'green' }}>
                        {success}
                    </Alert>
                )}
                ...
                <div style={{ marginTop: 24, padding: 16, backgroundColor: 'black', borderRadius: 8 }}>
                    ...
                    <ul style={{ margin: 8, paddingLeft: 20 }}>
                        <li><strong>Viewer:</strong> Can view posts, forms, and comments</li>
                        <li><strong>Editor:</strong> Can create/edit posts, manage tags, view admin panel</li>
                        <li><strong>OrgAdmin:</strong> Full access including user management and system settings</li>
                    </ul>
                </div>
            </Container>
        );
...
describe('CreateUserPage', () => {
    ...
    it('should render the page with correct title and description', () => {
        ...
        expect(screen.getByText('Add a new user to your organization with appropriate role permissions.')).toBeInTheDocument();
    });
    ...
    it('should successfully create user', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('User John Doe (test@example.com) created successfully with Viewer role!')).toBeInTheDocument();
        });
    });
    ...
    it('should display role permissions information', () => {
        ...
        expect(screen.getByText('Can create/edit posts, manage tags, view admin panel')).toBeInTheDocument();

        expect(screen.getByText('OrgAdmin:')).toBeInTheDocument();
        expect(screen.getByText('Full access including user management and system settings')).toBeInTheDocument();
    });
