...
jest.mock('./LoginPage', () => {
    ...
    return function MockAdminLoginPage() {
        ...
        if (token) {
            return <div data-testid="navigate" data-to="/" data-replace="true">Navigate to /</div>;
        }
        ...
        async function onSubmit(e: React.FormEvent) {
            ...
            try {
                ...
                const hasAdminRole = me?.memberships?.some((membership: any) =>
                    membership.role === 'Editor' || membership.role === 'OrgAdmin'
                );
        ...
        return (
            <Container maxWidth="360px">
                ...
                <div style={{ marginTop: 24, textAlign: 'center' }}>
                    <Button
                        variant="outline"
                        onClick={() => mockWindowLocationHref('http://localhost:4201')}
                    >
                        Back to Portal
                    </Button>
                </div>
...
describe('AdminLoginPage', () => {
    ...
    it('should render the login page with correct title and description', () => {
        ...
        expect(screen.getByText('Only Editors and OrgAdmins can access the admin panel.')).toBeInTheDocument();
    });
    ...
    it('should handle successful login with admin role', async () => {
        const mockUserData = {
            memberships: [
                { organizationId: 'org-123', role: 'Editor' }
            ]
        };
    ...
    it('should handle successful login with OrgAdmin role', async () => {
        const mockUserData = {
            memberships: [
                { organizationId: 'org-456', role: 'OrgAdmin' }
            ]
        };
    ...
    it('should show error for insufficient permissions', async () => {
        const mockUserData = {
            memberships: [
                { organizationId: 'org-123', role: 'Viewer' }
            ]
        };
        ...
        await waitFor(() => {
            expect(screen.getByText('Access denied. Only Editors and OrgAdmins can access the admin panel.')).toBeInTheDocument();
        });
    ...
    it('should clear error when form is submitted again', async () => {
        ...
        mockedAxios.get.mockResolvedValue({ data: { memberships: [{ organizationId: 'org-123', role: 'Editor' }] } });
