...
export class PostsService {
  ...
  async create(orgId: string, authorId: string, data: CreatePostDto) {
    ...
    if (tagIds.length) {
      ...
      if (count !== tagIds.length)
        throw new ForbiddenException('One or more tags do not belong to this organization');
    }
  ...
  async update(
    ...
    return await this.prisma.$transaction(async tx => {
      ...
      if (updated.count === 0)
        throw new ConflictException('Version conflict — please refresh and retry');
      ...
      if (dto.content) {
        ...
        await tx.revision.create({
          data: { postId: id, version: fresh!.version, content: dto.content },
        });
      }
