...
describe('PostsService', () => {
    ...
    describe('create', () => {
        ...
        it('should create a post successfully', async () => {
            ...
            expect(mockOutboxService.publish).toHaveBeenCalledWith('post.created', { id: expectedPost.id, orgId });
        ...
        it('should throw ForbiddenException when tags do not belong to organization', async () => {
            ...
            await expect(service.create(orgId, authorId, createData)).rejects.toThrow('One or more tags do not belong to this organization');
        });
    ...
    describe('update', () => {
        ...
        it('should throw NotFoundException when post not found', async () => {
            ...
            await expect(service.update(orgId, postId, userId, updateData)).rejects.toThrow(NotFoundException);
            await expect(service.update(orgId, postId, userId, updateData)).rejects.toThrow('Post not found');
        });
        ...
        it('should throw ForbiddenException when user has no membership', async () => {
            ...
            await expect(service.update(orgId, postId, userId, updateData)).rejects.toThrow(ForbiddenException);
            await expect(service.update(orgId, postId, userId, updateData)).rejects.toThrow('No membership for this organization');
        });
        ...
        it('should throw ForbiddenException when user cannot edit post', async () => {
            ...
            await expect(service.update(orgId, postId, userId, updateData)).rejects.toThrow(ForbiddenException);
            await expect(service.update(orgId, postId, userId, updateData)).rejects.toThrow('Only the author, Editor, or OrgAdmin can edit this post');
        });
        ...
        it('should throw ConflictException on version conflict', async () => {
            ...
            await expect(service.update(orgId, postId, userId, updateData)).rejects.toThrow(ConflictException);
            await expect(service.update(orgId, postId, userId, updateData)).rejects.toThrow('Version conflict — please refresh and retry');
        });
    ...
    describe('remove', () => {
        ...
        it('should throw ForbiddenException when user has no membership', async () => {
            ...
            await expect(service.remove(orgId, postId, userId)).rejects.toThrow('No membership for this organization');
        });
        ...
        it('should throw ForbiddenException when user cannot delete post', async () => {
            ...
            await expect(service.remove(orgId, postId, userId)).rejects.toThrow(ForbiddenException);
            await expect(service.remove(orgId, postId, userId)).rejects.toThrow('Only the author, Editor, or OrgAdmin can delete this post');
        });
    });
    ...
    describe('getRevision', () => {
        ...
        it('should throw NotFoundException when revision not found', async () => {
            ...
            await expect(service.getRevision(orgId, postId, version)).rejects.toThrow(NotFoundException);
            await expect(service.getRevision(orgId, postId, version)).rejects.toThrow('Revision not found');
        });
    });
    ...
    describe('rollbackToRevision', () => {
        ...
        it('should throw NotFoundException when post not found', async () => {
            ...
            await expect(service.rollbackToRevision(orgId, postId, userId, version)).rejects.toThrow(NotFoundException);
            await expect(service.rollbackToRevision(orgId, postId, userId, version)).rejects.toThrow('Post not found');
        });
        ...
        it('should throw NotFoundException when revision not found', async () => {
            ...
            await expect(service.rollbackToRevision(orgId, postId, userId, version)).rejects.toThrow(NotFoundException);
            await expect(service.rollbackToRevision(orgId, postId, userId, version)).rejects.toThrow('Revision not found');
        });
        ...
        it('should throw ForbiddenException when user cannot rollback', async () => {
            ...
            await expect(service.rollbackToRevision(orgId, postId, userId, version)).rejects.toThrow(ForbiddenException);
            await expect(service.rollbackToRevision(orgId, postId, userId, version)).rejects.toThrow('Only author, Editor, or OrgAdmin can rollback this post');
        });
    });
});
