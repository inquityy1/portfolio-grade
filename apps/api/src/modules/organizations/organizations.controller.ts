...
export class OrganizationsController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves all organizations in the system. Only users with OrgAdmin role in any organization can access this endpoint. Results are ordered by organization name.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - User does not have OrgAdmin role in any organization',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves detailed information about a specific organization including creation and update timestamps.',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiOperation({
    ...
    description:
      'Creates a new organization with the specified name. Automatically generates a URL-friendly ID and creates default system users with OrgAdmin, Editor, and Viewer roles. Organization names must be unique.',
  })
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        name: {
          ...
          description:
            'Organization name (2-100 characters, letters, numbers, spaces, hyphens, underscores only)',
        },
      },
      required: ['name'],
    },
