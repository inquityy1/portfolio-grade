...
export class AdminJobsController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves all tag usage statistics for the current organization, ordered by usage count descending. Returns tag ID, organization ID, count, and calculation timestamp.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Queues a background job to recalculate tag usage statistics for the current organization. This is useful when tag counts may be out of sync.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Generates a preview image for a specific post. If Redis is available, the job is queued for background processing. If not, the preview is generated synchronously.',
  })
  @ApiParam({
    name: 'postId',
