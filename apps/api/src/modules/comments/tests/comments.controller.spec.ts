...
describe('CommentsController', () => {
    ...
    describe('create', () => {
        ...
        it('should throw NotFoundException when post is not found', async () => {
            ...
            await expect(controller.create(orgId, postId, mockRequest, { content })).rejects.toThrow(NotFoundException);
            expect(service.create).toHaveBeenCalledWith(orgId, postId, 'user-123', content);
        });
    });
    ...
    describe('update', () => {
        ...
        it('should throw NotFoundException when comment is not found', async () => {
            ...
            await expect(controller.update(orgId, commentId, mockRequest, { content })).rejects.toThrow(NotFoundException);
            expect(service.update).toHaveBeenCalledWith(orgId, commentId, 'user-123', content);
        });
        ...
        it('should throw ForbiddenException when user is not authorized to update', async () => {
            ...
            mockCommentsService.update.mockRejectedValue(new ForbiddenException('Not allowed to edit this comment'));

            await expect(controller.update(orgId, commentId, mockRequest, { content })).rejects.toThrow(ForbiddenException);
            expect(service.update).toHaveBeenCalledWith(orgId, commentId, 'user-123', content);
        });
    });
    ...
    describe('remove', () => {
        ...
        it('should throw NotFoundException when comment is not found', async () => {
            ...
            await expect(controller.remove(orgId, commentId, mockRequest)).rejects.toThrow(NotFoundException);
            expect(service.remove).toHaveBeenCalledWith(orgId, commentId, 'user-123');
        });
        ...
        it('should throw ForbiddenException when user is not authorized to delete', async () => {
            ...
            mockCommentsService.remove.mockRejectedValue(new ForbiddenException('Not allowed to delete this comment'));

            await expect(controller.remove(orgId, commentId, mockRequest)).rejects.toThrow(ForbiddenException);
            expect(service.remove).toHaveBeenCalledWith(orgId, commentId, 'user-123');
        });
    });
    ...
    describe('restore', () => {
        ...
        it('should throw NotFoundException when comment is not found', async () => {
            ...
            await expect(controller.restore(orgId, commentId, mockRequest)).rejects.toThrow(NotFoundException);
            expect(service.restore).toHaveBeenCalledWith(orgId, commentId, 'user-123');
        });
        ...
        it('should throw ForbiddenException when user is not authorized to restore', async () => {
            ...
            mockCommentsService.restore.mockRejectedValue(new ForbiddenException('Only Editor or OrgAdmin can restore comments'));

            await expect(controller.restore(orgId, commentId, mockRequest)).rejects.toThrow(ForbiddenException);
            expect(service.restore).toHaveBeenCalledWith(orgId, commentId, 'user-123');
        });
    });
});
