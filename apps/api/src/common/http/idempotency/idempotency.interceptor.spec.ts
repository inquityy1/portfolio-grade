...
describe('IdempotencyInterceptor', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [IdempotencyInterceptor, { provide: PrismaService, useValue: mockPrismaValue }],
    }).compile();
  ...
  describe('intercept', () => {
    ...
    it('should handle different HTTP methods correctly', done => {
      ...
      methods.forEach(method => {
        ...
        (mockPrismaService.idempotencyKey.findUnique as jest.Mock).mockResolvedValue(null);
        (mockPrismaService.idempotencyKey.create as jest.Mock).mockResolvedValue({
          id: `key-${method}`,
        });
        (mockPrismaService.idempotencyKey.update as jest.Mock).mockResolvedValue({});
      ...
      interceptor.intercept(mockContext, mockCallHandler).subscribe({
        next: () => {
          expect(mockPrismaService.idempotencyKey.findUnique).toHaveBeenCalledWith({
            where: {
              orgId_route_key: { orgId: 'org-123', route: 'POST /api/posts', key: 'test-key-123' },
            },
    ...
    it('should handle route without baseUrl', done => {
      ...
      interceptor.intercept(mockContext, mockCallHandler).subscribe({
        next: () => {
          expect(mockPrismaService.idempotencyKey.findUnique).toHaveBeenCalledWith({
            where: {
              orgId_route_key: { orgId: 'org-123', route: 'POST /posts', key: 'test-key-123' },
            },
            select: { bodyHash: true, response: true },
          });
          done();
    ...
    it('should handle database errors gracefully during update', done => {
      (mockPrismaService.idempotencyKey.findUnique as jest.Mock).mockResolvedValue(null);
      (mockPrismaService.idempotencyKey.create as jest.Mock).mockResolvedValue({ id: 'key-123' });
      (mockPrismaService.idempotencyKey.update as jest.Mock).mockRejectedValue(
        new Error('DB Error'),
      );
    ...
    it('should handle database errors during findUnique', done => {
      (mockPrismaService.idempotencyKey.findUnique as jest.Mock).mockRejectedValue(
        new Error('DB Error'),
      );
    ...
    it('should handle database errors during create', done => {
      (mockPrismaService.idempotencyKey.findUnique as jest.Mock).mockResolvedValue(null);
      (mockPrismaService.idempotencyKey.create as jest.Mock).mockRejectedValue(
