...
describe('RateLimitGuard', () => {
  ...
  describe('canActivate', () => {
    ...
    it('should handle missing user', async () => {
      ...
      expect(result).toBe(true);
      expect(mockRateLimitService.hit).toHaveBeenCalledWith('user:anon:GET:/api/api/test', 60, 10);
    });
    ...
    it('should handle missing IP address', async () => {
      ...
      expect(result).toBe(true);
      expect(mockRateLimitService.hit).toHaveBeenCalledWith('ip:0.0.0.0:GET:/api/api/test', 5, 30);
    });
    ...
    it('should handle empty path and baseUrl', async () => {
      ...
      expect(result).toBe(true);
      expect(mockRateLimitService.hit).toHaveBeenCalledWith('user:user-123:GET:', 60, 10);
    });
  });
  ...
  describe('rate limit key generation', () => {
    it('should generate correct keys for different scopes', () => {
      ...
      testCases.forEach(({ scope, expectedKey }) => {
        ...
        const key = `${scope}:${
          scope === 'user' ? 'user-123' : scope === 'org' ? 'org-456' : '192.168.1.100'
        }:${
          scope === 'user'
            ? 'GET:/api/test'
            : scope === 'org'
