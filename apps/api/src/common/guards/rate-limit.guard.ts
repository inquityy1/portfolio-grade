...
export class RateLimitGuard implements CanActivate {
  constructor(private readonly reflector: Reflector, private readonly limiter: RateLimitService) {}
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    ...
    const cfg = this.reflector.get<RateLimitConfig>(RATE_LIMIT_KEY, ctx.getHandler()) ??
      this.reflector.get<RateLimitConfig>(RATE_LIMIT_KEY, ctx.getClass()) ?? {
        ...
        perUser: { limit: 60, windowSec: 10 },
        perOrg: { limit: 600, windowSec: 60 },
      };
    ...
    if (cfg.perUser) {
      ...
      checks.push(
        this.limiter
          .hit(key, cfg.perUser.limit, cfg.perUser.windowSec)
          .then(r => ({ scope: 'user', res: r })),
      );
    }
    ...
    if (cfg.perOrg) {
      ...
      checks.push(
        this.limiter
          .hit(key, cfg.perOrg.limit, cfg.perOrg.windowSec)
          .then(r => ({ scope: 'org', res: r })),
      );
    }
    ...
    if (cfg.perIp) {
      ...
      checks.push(
        this.limiter
          .hit(key, cfg.perIp.limit, cfg.perIp.windowSec)
          .then(r => ({ scope: 'ip', res: r })),
      );
    }
