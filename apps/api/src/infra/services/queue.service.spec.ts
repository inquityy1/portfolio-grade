...
describe('QueueService', () => {
    ...
    describe('addRepeatable method', () => {
        ...
        it('should add repeatable job with number interval', async () => {
            ...
            const result = await service.addRepeatable('test-queue', 'repeatable-job', { data: 'test' }, 5000);

            expect(result).toEqual({ id: 'repeatable-job-123' });
            expect(mockQueue.add).toHaveBeenCalledWith('repeatable-job', { data: 'test' }, {
                repeat: { every: 5000 },
            });
        });
        ...
        it('should add repeatable job with cron pattern', async () => {
            ...
            const result = await service.addRepeatable('test-queue', 'cron-job', { data: 'test' }, cronPattern);

            expect(result).toEqual({ id: 'cron-job-456' });
            expect(mockQueue.add).toHaveBeenCalledWith('cron-job', { data: 'test' }, {
        ...
        it('should add repeatable job with additional options', async () => {
            ...
            const result = await service.addRepeatable('test-queue', 'repeatable-job', { data: 'test' }, 3000, options);
        ...
        it('should handle different interval types', async () => {
            ...
            expect(mockQueue.add).toHaveBeenCalledWith('number-interval', { data: 'test' }, {
                repeat: { every: 1000 },
            });

            // Test with string (cron)
            await service.addRepeatable('test-queue', 'cron-interval', { data: 'test' }, '0 */5 * * *');
            expect(mockQueue.add).toHaveBeenCalledWith('cron-interval', { data: 'test' }, {
                repeat: { pattern: '0 */5 * * *' },
            });
        });
    });
