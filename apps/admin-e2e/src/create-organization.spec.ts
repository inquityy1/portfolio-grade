...
test.describe('Create Organization Page', () => {
    ...
    test('should display create organization page', async () => {
        ...
        await expect(helpers.page.locator('p')).toContainText('Create a new organization to manage users, posts, and forms');

        // Check form field
        await expect(helpers.page.locator('label:has-text("Organization Name *")')).toBeVisible();
        await expect(helpers.page.locator('input[placeholder="Enter organization name"]')).toBeVisible();
    });
    ...
    test('should have help text', async () => {
        ...
        // Check help text
        await expect(helpers.page.locator('small:has-text("Organization name must be 2-100 characters long")')).toBeVisible();
    });
