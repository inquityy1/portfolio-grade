...
test.describe('Create User Page', () => {
    ...
    test('should display create user page', async () => {
        ...
        await expect(helpers.page.locator('p')).toContainText('Add a new user to your organization with appropriate role permissions');
    ...
    test('should have role options', async () => {
        ...
        await expect(roleSelect.locator('option[value="Viewer"]')).toHaveText('Viewer - Can view content only');
        await expect(roleSelect.locator('option[value="Editor"]')).toHaveText('Editor - Can create and edit content');
        await expect(roleSelect.locator('option[value="OrgAdmin"]')).toHaveText('OrgAdmin - Full administrative access');
    });
