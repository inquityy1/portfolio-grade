...
jest.mock('./PostsPage', () => {
    ...
    const MockPostsPage = function MockPostsPage() {
        ...
        const canEditPosts = user?.memberships?.some((m: any) => m.role === 'Editor' || m.role === 'OrgAdmin');
        ...
        const submitRestore = async (e: React.FormEvent) => {
            ...
            try {
                setRestoring(true);
                const mockAxios = require('axios');
                const { data } = await mockAxios.post(`http://localhost:3000/api/comments/${restoreCommentId.trim()}/restore`, {}, {
                    headers: {
        ...
        const submitCreate = async (e: React.FormEvent) => {
            ...
            try {
                setSaving(true);
                const mockAxios = require('axios');
                await mockAxios.post('http://localhost:3000/api/posts', {
                    title,
                    content,
                    tagIds: []
                }, {
                ...
                setCreateOpen(false);
                setPosts([...posts, { id: 'new', title, content, author: { name: 'Current User' }, createdAt: new Date().toISOString(), tags: [] }]);
            } catch (err) {
        ...
        const submitEdit = async (e: React.FormEvent) => {
            ...
            try {
                setSaving(true);
                const mockAxios = require('axios');
                await mockAxios.patch(`http://localhost:3000/api/posts/${editing.id}`, {
                    title,
                    content,
            ...
            <section style={{ padding: 24, maxWidth: 800, margin: '0 auto' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <label data-testid="label" style={{ margin: 0, fontSize: 14 }}>Filter</label>
                            <select
                                data-testid="select"
                                value={selectedTagId}
                        ...
                        {canEditPosts && (
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button data-testid="button" onClick={openRestore}>Restore comment</button>
                                <button data-testid="button" onClick={openCreate}>Create new post</button>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        return (
                            <div key={p.id} style={{ border: '1px solid #2e2e2e', borderRadius: 16, padding: 16, background: 'rgba(255,255,255,0.02)' }}>
                                <header style={{ display: 'flex', gap: 12, alignItems: 'baseline', marginBottom: 8 }}>
                                    ...
                                    {canEditPosts && (
                                        <div style={{ display: 'flex', gap: 8 }}>
                                            <button data-testid="button" onClick={() => openEdit(p)}>Edit</button>
                                            <button data-testid="button" onClick={() => onDelete(p.id)} disabled={busyId === p.id}>
                                ...
                                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                                    <button data-testid="button" onClick={() => toggleComments(p.id)}>{open ? 'Hide comments' : 'Show comments'}</button>
                                </div>
                                                ...
                                                <div key={c.id} style={{ border: '1px solid #333', borderRadius: 12, padding: 10 }}>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                                                        <div style={{ fontSize: 13, opacity: 0.8 }}>
                                                            {c.author?.name ?? 'Anonymous'} · {c.createdAt ? new Date(c.createdAt).toLocaleString() : ''}
                                                        </div>
                                                    </div>
                        ...
                        <div data-testid="modal-content">
                            <form onSubmit={submitCreate} style={{ display: 'grid', gap: 12 }}>
                                <div data-testid="field">
                                    ...
                                    <input data-testid="input" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Post title" />
                                </div>
                                <div data-testid="field">
                                    <label data-testid="label">Content</label>
                                    <textarea data-testid="textarea" rows={8} value={content} onChange={(e) => setContent(e.target.value)} placeholder="Write your post…" />
                                </div>
                            </form>
                        </div>
                        <div data-testid="modal-footer">
                            <button data-testid="button" onClick={() => setCreateOpen(false)}>Cancel</button>
                            <button data-testid="button" onClick={submitCreate} disabled={saving}>{saving ? 'Creating…' : 'Create'}</button>
                        </div>
                        <button data-testid="modal-close" onClick={() => setCreateOpen(false)}>Close</button>
                    </div>
                )}
                ...
                {editOpen && (
                    <div data-testid="modal">
                        ...
                        <div data-testid="modal-footer">
                            <button data-testid="button" onClick={() => { setEditOpen(false); setEditing(null); }}>Cancel</button>
                            <button data-testid="button" onClick={submitEdit} disabled={saving}>{saving ? 'Saving…' : 'Save'}</button>
                        </div>
                        <button data-testid="modal-close" onClick={() => { setEditOpen(false); setEditing(null); }}>Close</button>
                    </div>
                )}
                ...
                {restoreOpen && (
                    <div data-testid="modal">
                        ...
                        <div data-testid="modal-footer">
                            <button data-testid="button" onClick={() => setRestoreOpen(false)}>Cancel</button>
                            <button data-testid="button" onClick={submitRestore} disabled={restoring || !restoreCommentId.trim()}>
                                {restoring ? 'Restoring…' : 'Restore'}
                            </button>
                        </div>
                        <button data-testid="modal-close" onClick={() => setRestoreOpen(false)}>Close</button>
                    </div>
                )}
            </section>
        );
