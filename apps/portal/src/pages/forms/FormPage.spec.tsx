  ...
  const MockFormPage = function MockFormPage() {
    ...
    mockReact.useEffect(() => {
      ...
      (async () => {
        try {
          ...
          const axiosCall = mockAxios.get(`http://localhost:3000/api/public/forms/${id}`, {
            headers,
          });
    ...
    const fields = mockReact.useMemo(() => {
      ...
      return Object.keys(props).map((key, i) => {
        ...
        const isTA =
          typeStr === 'string' &&
          (def?.format === 'multiline' || /message|comment|description/i.test(key));
        return {
          id: key,
    ...
    const handleSubmit = async (values: any) => {
      ...
      try {
        ...
        const axiosCall = mockAxios.post(
          `http://localhost:3000/api/public/forms/${id}/submit`,
          { data: values },
          { headers },
        );
        if (axiosCall && axiosCall.then) {
    ...
    if (err)
      return (
        <div data-testid='error' data-variant='error'>
          {err}
        </div>
      );
    if (!form)
      return (
        <div data-testid='not-found' style={{ padding: 24 }}>
          Form not found
        </div>
      );
