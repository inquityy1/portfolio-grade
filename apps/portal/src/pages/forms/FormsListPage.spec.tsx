...
jest.mock('./FormsListPage', () => {
    ...
    const MockFormsListPage = function MockFormsListPage() {
        ...
        const token = mockLocalStorageGetItem('token') || mockLocalStorageGetItem('accessToken') || null;
        ...
        return (
            <div data-testid="container">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    ...
                    {canEdit && (
                        <button data-testid="create-button" onClick={goCreate}>Create new form</button>
                    )}
                </div>

                {flashMessage && (
                    <div data-testid="flash-message" style={{
                        color: 'green',
                ...
                {!error && (!items || items.length === 0) && <div data-testid="empty-state">Sorry but there is no forms right now</div>}

                {items && items.length > 0 && !error && (
                    <div data-testid="forms-grid" style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
                        {items.map((f: any) => (
                            <div key={f.id} data-testid={`form-card-${f.id}`}>
                                <a href={`/forms/${f.id}`} data-testid={`form-link-${f.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                                    <div data-testid="card-header">
                                        <h3 data-testid="card-title">{f.name}</h3>
                                        <span data-testid="status-badge" style={{ fontSize: 12, border: '1px solid #2e2e2e', borderRadius: 999, padding: '2px 8px', opacity: 0.7 }}>
                                            {f.status ?? 'draft'}
                                        </span>
                                    </div>
                                    <div data-testid="card-content">
                                        {f.description && <p data-testid="description" style={{ margin: '8px 0 0', opacity: 0.7 }}>{f.description}</p>}
                                        {f.updatedAt && (
                                            <p data-testid="updated-at" style={{ margin: '8px 0 0', fontSize: 12, opacity: 0.6 }}>
...
describe('FormsListPage', () => {
            ...
            if (url.includes('/auth/me')) {
                return Promise.resolve({
                    data: {
                        id: 'user-1',
                        email: 'test@example.com',
                        memberships: [{ organizationId: 'org-1', role: 'Editor', organization: { name: 'Test Org' } }]
                    }
                });
            }
    ...
    it('should not show create button for users without editor rights', async () => {
        const mockUserData = {
            id: 'user-1',
            email: 'test@example.com',
            memberships: [{ organizationId: 'org-1', role: 'Viewer', organization: { name: 'Test Org' } }]
        };
    ...
    it('should navigate to edit form page', async () => {
        const mockFormsData = {
            items: [{ id: '1', name: 'Test Form', description: 'Test', status: 'draft', updatedAt: null }]
        };
    ...
    it('should handle different API response formats', async () => {
        ...
        const mockFormsData = [
            ...
            { id: '2', name: 'Test Form 2', description: 'Test', status: 'published', updatedAt: '2023-01-01T00:00:00Z' }
        ];
